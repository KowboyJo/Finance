...
exclude_earnings = st.sidebar.checkbox(
    ...
    value=True,
    help="Filters out tickers that have an earnings release scheduled before option expiration.",
)
...
min_open_interest = st.sidebar.number_input(
    ...
    value=100,
    step=25,
    help="Filters out illiquid option contracts with low open contract volume.",
)
        ...
        if not passed_tickers:
            ...
            if results:
                ...
                st.info(
                    ...
                    f"• **Liquidity Check:** Open Interest is {top_trade['Open Interest']} contracts.\n\n"
                    f"• **Tactical Edge:** Optimal balance of volatility capture and theta decay velocity near major moving average support."
                )
            else:
                st.warning(
                    "No options found matching your parameters, spread criteria, open interest, or earnings filters."
                )
else:
