...
# --- MAIN EXECUTION FLOW ---
if run_button:
  # Added main-panel spinner for mobile visibility
  with st.spinner(
      "🔍 Running deep scan across market universe & option chains..."
  ):
    universe = fetch_universe()
    st.info(
        f"Loaded {len(universe)} symbols. Screening for fundamentals &"
        " support..."
    )
    ...
    status_text.text(
        f"Screening complete! Found {len(passed_tickers)} matching equities near"
        " support."
    )
    ...
    if not passed_tickers:
      ...
      st.success(
          "Scanning option chains & applying risk filters for"
          f" {len(passed_tickers)} matches..."
      )
      ...
      if results:
        ...
        st.info(
            ...
            f" ({top_trade['Cycle']})\n\n• **Current Stock Price:**"
            f" ${top_trade['Stock Price']}\n\n• **Optimal Strike:**"
            f" ${top_trade['Strike']} (IV: {top_trade['IV (%)']}%)\n\n• **Put"
            f" Premium (Mid):** ${top_trade['Put Premium']}\n\n• **Capital"
            f" Efficiency:** Yields {top_trade['Ratio (%)']}% return over"
            f" {top_trade['DTE']} days.\n\n• **Tactical Edge:** Optimal balance"
            " of volatility capture and theta decay velocity near major moving"
            " average support."
        )
      else:
        st.warning(
