...
# ================================================================
# SIDEBAR (Condensed Layout)
# ================================================================

# Tighten sidebar padding and decrease widget spacing
st.markdown(
    """
    <style>
...
st.sidebar.markdown("### 📈 Screener Parameters")

# --- Fundamentals Section ---
st.sidebar.markdown("**Fundamentals**")
f_col1, f_col2 = st.sidebar.columns(2)
with f_col1:
    max_pe = st.number_input("Max P/E", min_value=1.0, max_value=200.0, value=40.0, step=1.0)
    min_revenue = st.number_input("Min Rev ($B)", min_value=0.0, value=3.0, step=0.5)
with f_col2:
    max_pct_support = st.number_input("Max % > 200SMA", min_value=-50.0, max_value=100.0, value=20.0, step=1.0)
    min_cash = st.number_input("Min Cash ($B)", min_value=0.0, value=1.0, step=0.5)

f_sub1, f_sub2 = st.sidebar.columns([1.2, 1])
with f_sub1:
    max_debt_equity = st.number_input("Max D/E (%)", min_value=0.0, value=150.0, step=10.0)
with f_sub2:
    st.markdown("<div style='height: 22px;'></div>", unsafe_allow_html=True)
    require_positive_fcf = st.checkbox("Require FCF", value=True)

# --- Option Selection Section ---
st.sidebar.markdown("**Option Selection**")
o_col1, o_col2 = st.sidebar.columns(2)
with o_col1:
    target_delta = st.number_input("Target Delta", min_value=0.05, max_value=0.80, value=0.25, step=0.05, format="%.2f")
    min_open_interest = st.number_input("Min OI", min_value=0, value=100, step=50)
with o_col2:
    max_spread_pct = st.number_input("Max Spread %", min_value=1.0, max_value=50.0, value=12.0, step=1.0)
    min_volume = st.number_input("Min Vol", min_value=0, value=5, step=5)

# --- Risk & Expiration Section ---
st.sidebar.markdown("**Risk & Expiration**")
r_col1, r_col2 = st.sidebar.columns(2)
with r_col1:
    exclude_earnings = st.checkbox("No Earnings", value=True)
    use_custom_exp = st.checkbox("Custom Exp", value=False)
with r_col2:
    risk_free_rate = st.number_input("Risk-Free %", min_value=0.0, max_value=15.0, value=4.0, step=0.25) / 100.0

target_expiration = None
if use_custom_exp:
    exp_date_input = st.sidebar.date_input("Expiration Date")
