...
def analyze_puts(
    ...
    """Analyzes put option chains with resilient liquidity checks."""
    ...
    try:
        ...
        # Open Interest Guardrail with Bid/Ask Fallback
        if oi < min_oi and not (bid > 0 and ask > 0):
            return None
                ...
                try:
                    ...
                    if target_expiration:
                        if target_expiration in exp_dates:
                            ...
                            closest_exp = min(
                                exp_dates,
                                key=lambda d: abs((datetime.strptime(d, "%Y-%m-%d") - datetime.strptime(target_expiration, "%Y-%m-%d")).days)
                            )
                            res = analyze_puts(
                                ticker,
                                closest_exp,
                                spot_price,
                                f"Closest ({closest_exp})",
                                target_delta,
                                max_spread=max_spread_pct,
                                check_earnings=effective_exclude_earnings,
                                min_oi=min_open_interest,
