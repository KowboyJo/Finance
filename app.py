...
st.markdown(
    "Scan the S&P 500 universe for high-efficiency, multi-exchange income"
    " opportunities."
)
...
max_pe = st.sidebar.number_input("Max P/E Ratio", value=50.0, step=1.0)
max_pct_low = st.sidebar.number_input(
    "Max % Above 52-Week Low", value=10.0, step=1.0
)
target_delta = st.sidebar.number_input(
    "Target Put Delta (OTM)", value=0.20, step=0.05, format="%.2f"
)

# Expiration input handling
...
@st.cache_data(ttl=86400)  # Cache the universe list for 24 hours to speed up app
def fetch_universe():
  try:
...
if run_button:
  ...
  st.info(
      f"Loaded {len(universe)} symbols from large-cap universe. Running"
      " screener..."
  )
  ...
  for i, ticker in enumerate(universe):
    ...
    try:
      ...
      pe_ratio = info.get("trailingPE", info.get("forwardPE", 999))
      revenue = info.get("totalRevenue", 0)
      total_cash = info.get("totalCash", 0)

      if not price or not fifty_two_low or not pe_ratio:
        continue

      pct_above_low = ((price - fifty_two_low) / fifty_two_low) * 100

      if (
          pe_ratio <= max_pe
          and pe_ratio >= 0
          and pct_above_low <= max_pct_low
  ...
  status_text.text(
      f"Screening complete! Found {len(passed_tickers)} matching equities."
  )

  if not passed_tickers:
    st.warning("No stocks matched your custom thresholds.")
  else:
