...
if scan_mode == "Mag 7 Only":
    st.sidebar.caption("ℹ️ *Max P/E filter is bypassed for Mag 7 mode (P/E will still display).*")
...
if run_button:
    with st.spinner("🔍 Running deep scan across market universe & option chains..."):
        # Select target universe based on sidebar toggle
        if scan_mode == "Mag 7 Only":
        ...
        for i, ticker in enumerate(universe):
            ...
            try:
                ...
                # Bypass P/E requirement if scanning Mag 7 mode
        ...
        if not passed_tickers:
            ...
            if results:
                ...
                pe_display_text = f"P/E Ratio: {top_trade.get('P/E', 'N/A')}" if 'P/E' in top_trade else ""
                
                st.info(
                    f"• **Top Recommendation:** [{top_trade['Ticker']}](https://finance.yahoo.com/chart/{top_trade['Ticker']}) ({top_trade['Cycle']})\n\n"
                    f"• **Current Stock Price:** ${top_trade['Stock Price']} ({pe_display_text})\n\n"
