...
st.title("📈 Large-Cap & ETF Cash-Secured Put (CSP) Income Screener")
st.caption("Scan equities and ETFs for high-efficiency income near technical support.")

# --- ASSET PRESETS ---
MAG_7_TICKERS = ["AAPL", "MSFT", "GOOGL", "AMZN", "NVDA", "META", "TSLA"]
ETF_TICKERS = ["SOXL", "TQQQ", "QQQ", "SPY", "SMH", "IWM", "XLE", "SCHD"]
...
scan_mode = st.sidebar.radio(
    "Target Asset Class",
    options=["Mag 7", "ETFs", "Equities", "Full Universe"],
    index=0,
    help="Select an asset mode to target specific universes or ETF instruments like $SOXL.",
)
...
if scan_mode in ["Mag 7", "ETFs"]:
    st.sidebar.caption("💡 *P/E & Revenue filters are bypassed for Mag 7 and ETF modes.*")
...
with st.sidebar.expander("🛠️ Advanced Risk & Options Filters", expanded=False):
    exclude_earnings = st.checkbox(
        ...
        value=True,
        help="Filters out equities with earnings before expiration (ignored for ETFs).",
    )
...
with st.sidebar.expander("📊 Active Quality Thresholds", expanded=False):
    ...
    st.text("• Allowed Quotes: EQUITY, ETF")

run_button = st.sidebar.button("🚀 Run Scanner", type="primary", use_container_width=True)

# --- MAIN EXECUTION FLOW ---
if run_button:
    with st.spinner("🔍 Running deep scan across selected tickers & option chains..."):
        
        # Route universe selection based on active switch
        if scan_mode == "Mag 7":
        ...
        st.info(f"Loaded {len(universe)} symbols ({scan_mode} mode). Screening support & metrics...")
        ...
        for i, ticker in enumerate(universe):
            ...
            try:
                ...
                if is_etf:
                    pe_ratio = None
                    revenue = min_revenue  # Auto-pass for ETFs
                else:
                ...
                # Bypass P/E requirement for Mag 7 mode or ETFs
                pe_pass = True if (is_etf or scan_mode in ["Mag 7", "ETFs"]) else (pe_ratio <= max_pe)
        ...
        if not passed_tickers:
            st.warning("No tickers matched your parameters near support.")
        else:
            st.success(
            ...
            if results:
                ...
                st.warning(
                    "No option contracts found matching your parameters or spread filters."
                )
else:
