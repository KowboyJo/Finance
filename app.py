...
if scan_mode == "Mag 7 Only":
    st.sidebar.caption("💡 *P/E filter is bypassed for Mag 7 (displayed in results).*")

# Advanced Risk Controls inside an Expander (Keeps sidebar short on Mobile)
    ...
    with st.spinner("🔍 Running deep scan across market universe & option chains..."):
        ...
        if not passed_tickers:
            ...
            if results:
                ...
                pe_display_text = (
                    f"P/E Ratio: {top_trade.get('P/E', 'N/A')}"
                    if "P/E" in top_trade
                    else ""
                )
