...
# Replaced 52-week low with Support Proximity (Distance from 200-Day SMA)
...
if run_button:
  ...
  for i, ticker in enumerate(universe):
    ...
    try:
      ...
      if not price or not pe_ratio:
        continue

      # Fetch historical data to compute the 200-day Moving Average (Support)
      hist = stock.history(period="1yr")
      if len(hist) < 200:
        continue
      sma_200 = hist["Close"].rolling(window=200).mean().iloc[-1]

      # Calculate % distance above the 200-day support line
      pct_above_support = ((price - sma_200) / sma_200) * 100

      # Filter: Must be above or near support (e.g., within max_pct_support of the 200 SMA, and not excessively overextended)
      # We allow it to be slightly below (-2%) down to +max_pct_support above support
      if (
          pe_ratio <= max_pe
          and pe_ratio >= 0
          and -2.0 <= pct_above_support <= max_pct_support
