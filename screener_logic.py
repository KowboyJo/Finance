...
def check_earnings_conflict(ticker_symbol, exp_date):
    """
    Safely checks if an upcoming earnings date occurs on or before the expiration date.
    Strips timezone data to avoid comparison crashes.
    """
    try:
...
def analyze_puts(
    ...
    Applies bid-ask spread filters, safe open interest checks, and calculates Yield (%).
    """
    ...
    try:
        ...
        # 2. Open Interest Extraction (Handles NaNs cleanly)
